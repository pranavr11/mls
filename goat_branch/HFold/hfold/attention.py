...
class HFoldAttention(nn.Module):
    ...
    def forward(
        ...
        if hidden_states.dim() != 3:
            msg = "hidden_states must have shape [batch, seq, d_model]."
            raise ValueError(msg)

        batch_size, seq_len, _ = hidden_states.shape
        if attention_mask is not None:
            if attention_mask.shape != (batch_size, seq_len):
                msg = "attention_mask must have shape [batch, seq]."
                raise ValueError(msg)
            token_mask = attention_mask.to(dtype=torch.bool, device=hidden_states.device)
        else:
        ...
        for index in range(seq_len):
            ...
            removed_positions = torch.cat(
                [
                    popped["positions"],
                    overflow["positions"],
                    next_local["aged_out_positions"].unsqueeze(1).expand(-1, self.config.n_heads, -1),
                ],
                dim=2,
            )
            ...
            folded_state = self._fold_removed_memory_state(
                state=popped["state"],
            ...
            folded_state.last_candidate_counts = local_candidates["valid"].sum(dim=-1)
            folded_state.last_retrieved_counts = popped["counts"]
            folded_state.last_removed_counts = removed_valid.sum(dim=-1)
            folded_state.last_evicted_counts = overflow["counts"]
            state = folded_state
        ...
        merged = self._merge_heads(context_tensor)
        output = self.resid_dropout(self.out_proj(merged))
    ...
    def _init_state(
        ...
        *,
        ...
        device: torch.device,
        dtype: torch.dtype,
        cache: Optional[HFoldMemoryState],
    ) -> HFoldMemoryState:
        if cache is None:
    ...
    def _split_heads(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        return tensor.view(batch_size, seq_len, self.config.n_heads, self.head_dim).transpose(1, 2)

    def _merge_heads(self, tensor: torch.Tensor) -> torch.Tensor:
        batch_size, _, seq_len, _ = tensor.shape
        return tensor.transpose(1, 2).contiguous().view(batch_size, seq_len, self.config.d_model)
    ...
    def _build_local_context(self, *, state: HFoldMemoryState) -> dict[str, torch.Tensor]:
        return {
            ...
            "valid": state.local_valid_mask.unsqueeze(1).expand(-1, self.config.n_heads, -1),
        }
    ...
    def _select_local_candidates(
        ...
        *,
        ...
        candidate_count = min(self.config.top_q, local_states.size(2))
        batch_size = local_states.size(0)
        if candidate_count == 0:
            empty_states = local_states.new_zeros(batch_size, self.config.n_heads, 0, self.head_dim)
            empty_scores = local_attn_weights.new_zeros(batch_size, self.config.n_heads, 0)
            empty_positions = local_positions.new_zeros(batch_size, self.config.n_heads, 0)
            empty_valid = local_valid.new_zeros(batch_size, self.config.n_heads, 0)
            return {
                "states": empty_states,
                "scores": empty_scores,
        ...
        candidate_mask = self._candidate_mask(
            combined_positions=local_positions.unsqueeze(1).expand(-1, self.config.n_heads, -1),
        ...
        candidate_positions = torch.gather(
            local_positions.unsqueeze(1).expand(-1, self.config.n_heads, -1),
            dim=2,
    ...
    def _advance_local_window(
        ...
        *,
        ...
        for batch_index in range(batch_size):
            ...
            if packed_positions.numel() > window_size:
                aged_out_count = packed_positions.numel() - window_size
                ...
                packed_keys = packed_keys[:, -window_size:, :]
                packed_values = packed_values[:, -window_size:, :]
                packed_positions = packed_positions[-window_size:]
    ...
    def _insert_candidates_into_memory(
        ...
        *,
        ...
        if self.config.heap_size == 0:
            empty_states = candidate_states.new_zeros(batch_size, self.config.n_heads, overflow_slots, self.head_dim)
            empty_scores = candidate_scores.new_zeros(batch_size, self.config.n_heads, overflow_slots)
            empty_positions = candidate_positions.new_full(
                (batch_size, self.config.n_heads, overflow_slots),
                fill_value=-1,
            )
            empty_valid = candidate_valid_mask.new_zeros(batch_size, self.config.n_heads, overflow_slots)
            return state, {
                "states": empty_states,
                "scores": empty_scores,
                "positions": empty_positions,
        ...
        overflow_states = candidate_states.new_zeros(batch_size, self.config.n_heads, overflow_slots, self.head_dim)
        overflow_scores = candidate_scores.new_zeros(batch_size, self.config.n_heads, overflow_slots)
        overflow_positions = candidate_positions.new_full(
            (batch_size, self.config.n_heads, overflow_slots),
            fill_value=-1,
        )
        overflow_valid = candidate_valid_mask.new_zeros(batch_size, self.config.n_heads, overflow_slots)
        ...
        for batch_index in range(batch_size):
            for head_index in range(self.config.n_heads):
                ...
                retained_count = min(self.config.heap_size, pool_scores.numel())
    ...
    def _pop_memory(
        ...
        *,
        ...
        batch_size = state.memory_states.size(0)
        pop_count = min(self.config.retrieve_e, self.config.heap_size)
        empty_counts = state.memory_positions.new_zeros(batch_size, self.config.n_heads)

        if pop_count == 0:
            empty_states = state.memory_states.new_zeros(
                batch_size,
                self.config.n_heads,
                0,
                self.head_dim,
            )
            empty_valid = state.memory_valid_mask.new_zeros(batch_size, self.config.n_heads, 0)
            return {
                "state": state,
                "states": empty_states,
                "keys": empty_states.clone(),
                "values": empty_states.clone(),
                "scores": state.memory_scores.new_zeros(batch_size, self.config.n_heads, 0),
                "positions": state.memory_positions.new_zeros(batch_size, self.config.n_heads, 0),
                "valid": empty_valid,
                "counts": empty_counts,
            }

        popped_states = state.memory_states.new_zeros(
            batch_size,
            self.config.n_heads,
            pop_count,
            self.head_dim,
        )
        popped_scores = state.memory_scores.new_zeros(batch_size, self.config.n_heads, pop_count)
        popped_positions = state.memory_positions.new_full((batch_size, self.config.n_heads, pop_count), -1)
        popped_valid = state.memory_valid_mask.new_zeros(batch_size, self.config.n_heads, pop_count)
